...
def checkOverlap(ivai, ivbi, ivaj, ivbj, margin=None):
    ...
    """
    if checkOneEndOverlap(ivai[0], ivai[1], ivaj[0],
                          ivaj[1]) and checkOneEndOverlap(
                              ivbi[0], ivbi[1], ivbj[0], ivbj[1]):
        return True
    if margin != None:
        if checkOneEndOverlap(ivai[0], ivai[1], ivaj[0] - margin,
                              ivaj[1] - margin) and checkOneEndOverlap(
                                  ivbi[0], ivbi[1], ivbj[0], ivbj[1]):
            return True
        if checkOneEndOverlap(ivai[0], ivai[1], ivaj[0] + margin,
                              ivaj[1] + margin) and checkOneEndOverlap(
    ...
    for i in range(len(rs)):
        ...
        for j in range(i + 1, len(rs)):
            if j in skips:
                continue
            nrj = rs[j]
            if checkOverlap([nr[0], nr[1]], [nr[2], nr[3]], [nrj[0], nrj[1]],
                            [nrj[2], nrj[3]], margin):
                skips.add(j)
                nr[0] = min([nr[0], nrj[0]])
                nr[1] = max([nr[1], nrj[1]])
